...
module.exports = {
    ...
    async run(client, interaction) {
        ...
        if(target.id == interaction.user.id){
            responseString = `<@${interaction.user.id}> slapped themselves!`;
        }
        //If they choosed the bot, make the message different
        if(target.id == client.user.id){
            responseString = `<@${interaction.user.id}> slapped me... :c`;
        }
        //If they choosed someone else
        if(responseString == ""){
            responseString = `<@${interaction.user.id}> slapped <@${target.id}>`;
        }
        //Get the slap gif
